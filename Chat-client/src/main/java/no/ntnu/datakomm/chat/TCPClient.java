...
public class TCPClient {
    ...
    private void parseIncomingCommands() {
        while (isConnectionActive()) {
            ...
            String resp = this.waitServerResponse();

            String[] splitStr = resp.split(" ", 3);
            switch (splitStr[0]){
                case "msg":
                    try {
                        onMsgReceived(false, splitStr[1], splitStr[2]);
                    } catch (ArrayIndexOutOfBoundsException e){}
                    break;

                case "privmsg":
                    onMsgReceived(true, splitStr[1], splitStr[2]);
                    break;
                case "msgerr":
                    onMsgError(splitStr[1]);
                    break;
                case "loginok":
                    onLoginResult(true, null);
                    break;
                case "loginerr":
                    onLoginResult(false, splitStr[1]);
                    break;
                case "users":
                    String[] users = resp.substring(6).split(" ");
                    onUsersList(users);
                    break;
                case "cmderr":
                    onCmdError(resp.substring(6));
                    break;
                case "supported":
                    String[] command = resp.substring(10).split(" ");
                    onSupported(command);
                    break;
            }
        }
